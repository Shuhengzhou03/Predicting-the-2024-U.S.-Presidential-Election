...
We will aim for a sample size of approximately 5,000 respondents, using a **stratified random sampling** method, to ensure that our sample is representative of the U.S. voting population across major demographic groups. Stratified sampling involves dividing the population into distinct subgroups, or "strata" (e.g., by age, gender, race, education, and geographic region). Within each stratum, respondents are randomly sampled in proportion to their presence in the population, allowing us to capture opinions from all significant subgroups.

**Strengths and Weaknesses of Stratified Sampling:**  

Stratified sampling reduces sampling error and increases the accuracy of estimates within each subgroup, making it especially useful for election forecasting, where certain demographics may have distinct voting patterns. However, stratified sampling requires accurate data on population characteristics to set the correct quotas, and oversampling smaller groups can increase both complexity and costs.
...
**Considerations for Recruitment Bias:**  

Panel respondents may exhibit panel bias, as frequent survey takers might not fully represent the broader population. To address this, we will use social media ads to reach unique respondents and adjust quotas as necessary to align with target demographic proportions.
